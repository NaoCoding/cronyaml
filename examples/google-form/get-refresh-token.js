...
/** One-time helper for a Google Forms read-only OAuth refresh token. */
...
const server = createServer(async (request, response) => {
  ...
  if (error || !code) {
    ...
    server.close();
    console.error(`[Google Forms] Authorization failed: ${error ?? "no authorization code"}`);
    process.exitCode = 1;
    return;
  }
  ...
  if (!tokenResponse.ok || typeof tokenResult.refresh_token !== "string") {
    console.error(`[Google Forms] Token exchange failed (${tokenResponse.status}): ${JSON.stringify(tokenResult)}`);
    process.exitCode = 1;
    return;
  }

  console.log("\nAdd this value to examples/google-form/.env:");
  console.log(`GOOGLE_REFRESH_TOKEN=${tokenResult.refresh_token}`);
});

server.listen(0, "127.0.0.1", () => {
  const address = server.address();
  if (!address || typeof address === "string") {
    console.error("Could not determine the local callback address.");
    process.exit(1);
  }
